...
const TbodyS = ({ rows = [], columns }) => {
    ...
    return (
        ...
        <tbody>
            {
                rows.filter((row) => (filter === "" || row.type === filter) && (text === "" || (row.type.toLowerCase().includes(text.toLowerCase())
                    ||
                    row.UrlVideo.toLowerCase().includes(text.toLowerCase())
                    ||
                    castDateToCrDate(row.date).toLowerCase().includes(text.toLowerCase()))))
                    .map((row, index) => {
                        ...
                        return (
                            <tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" onDoubleClick={handleDoubleClick}>
                                ...
                                <td className="px-6 py-4">
                                    <ButtonEdit title="session" data={{ id, inputDate, type, UrlVideo, report }}  >
